    ...
    <div v-if="filteredVideos.length > 0 && viewMode === 'list'" class="video-list">
      ...
      >
        <div class="col-name">
          ...
          <span class="video-name-text" :title="video.original_name">{{ video.original_name }}</span>
        </div>
        <span class="col-format">{{ video.format.toUpperCase() }}</span>
        <span class="col-size">{{ formatSize(video.size) }}</span>
        <span class="col-group">{{ getGroupName(video.group_id) || '-' }}</span>
        <span class="col-date">{{ formatDate(video.created_at) }}</span>
        <div class="col-actions">
          <button class="action-btn-small edit" @click="editVideo(video)" title="修改名称">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>
              <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>
            </svg>
          </button>
          <button class="action-btn-small group" @click="showChangeGroup(video)" title="切换分组">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/>
            </svg>
          </button>
          <button class="action-btn-small play" @click="playVideo(video)" title="播放">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polygon points="5 3 19 12 5 21 5 3"/>
            </svg>
          </button>
          <button class="action-btn-small delete" @click="confirmDelete(video)" title="删除">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <polyline points="3 6 5 6 21 6"/>
              <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
            </svg>
